...
@tf.function
def training_step_dqnet_target_critic_2(
        batch: ReplayHistoryType,
        minibatch_size: int,
        train_iterations: int,
        discount_rate: float,
...
#@tf.function
def training_step_dqnet_target_critic_ps(
        batch: ReplayHistoryType,
        weights: tf.Tensor,
        discount_rate: float,
        target_model: tf.keras.Model,
    ...
    with tf.GradientTape() as tape:
        all_Q_values, values = actor_model(states, training=True) # type: ignore
        Q_values = tf.reduce_sum(all_Q_values * mask, axis=1, keepdims=True)
        actor_loss = tf.reduce_mean(loss_fn(target_Q_values, Q_values)*weights)
        critic_loss = tf.reduce_mean(loss_fn(target_Q_values, values)*weights)

        td_error = tf.abs(target_Q_values - Q_values)
...
@tf.function
def training_step_dqnet_target_critic_state_transform(
        batch: ReplayHistoryType,
        minibatch_size: int,
        train_iterations: int,
        discount_rate: float,
    ...
    for _ in tf.range(train_iterations):
        ...
        best_next_actions = tf.argmax(next_Q_values, axis=1)
        next_mask = tf.one_hot(best_next_actions, n_outputs)
        next_states_transformed = tf_transform_state(next_states)
        ...
        with tf.GradientTape() as tape:
            all_Q_values, values = actor_model(states_transformed, training=True) # type: ignore
            Q_values = tf.reduce_sum(all_Q_values * mask, axis=1, keepdims=True)
            actor_loss = tf.reduce_mean(loss_fn(target_Q_values, Q_values))
            critic_loss = tf.reduce_mean(loss_fn(target_Q_values, values))
