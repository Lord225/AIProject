...
reshaped = tf.keras.layers.Reshape((8, 8, 8))(inputs)
x = tf.keras.layers.Conv2D(32,  3, activation="relu", padding='same')(reshaped)
...
critic = tf.keras.layers.Dense(1, tf.tanh)(critic)
critic = tf.keras.Model(inputs=inputs, outputs=critic)
...
for epoch in range(epochs):
    ...
    for t in range(steps_per_epoch):
        ...
        observation_new, reward, done = env_step(action[0].numpy())
